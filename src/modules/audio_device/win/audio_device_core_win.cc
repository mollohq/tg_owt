...
HRESULT
WaitForASyncWithEvent(_In_ IAsyncInfo *async_info,
...
template <DeviceClass DEVICE_CLASS> struct DeviceHelper {
protected:
...
public:
  ...
  int32_t DeviceName(uint16_t index, char name[webrtc::kAdmMaxDeviceNameSize],
  ...
  int32_t
  SetDevice(webrtc::AudioDeviceModule::WindowsDeviceType windowsDeviceType) {
    HRESULT hr;
...
bool AudioDeviceWindowsCore::CoreAudioIsSupported() { return true; }
...
int32_t
AudioDeviceWindowsCore::PlayoutDeviceName(uint16_t index,
...
int32_t
AudioDeviceWindowsCore::RecordingDeviceName(uint16_t index,
                                            char name[kAdmMaxDeviceNameSize],
                                            char guid[kAdmMaxGuidSize]) {
  webrtc::MutexLock lock(&_internal->_critSect);
...
bool AudioDeviceWindowsCore::BuiltInAECIsAvailable() const { return false; }

int32_t AudioDeviceWindowsCore::EnableBuiltInAEC(bool enable) { return -1; }
} // namespace webrtc
